...
export class Setup extends React.Component<ISetupProps, ISetupState> {
  ...
  public handleChange(event: React.ChangeEvent<HTMLElement>): void {
    switch (event.target.id) {
      ...
      case 'number-lives':
        ...
        this.setState({ numLives: numLives });
        break;
  ...
  render() {
    return(
      <div id='setup' style={Styles.setup}>
        <Difficulty value={this.state.difficulty} onChange={this.handleChange}/>
        <FlagsEnabled checked={this.state.flagsEnabled}
          onChange={this.handleChange}/>
        <NumberLives value={this.state.numLives} onChange={this.handleChange}/>
        <GameSeed value={this.state.gameSeed} onChange={this.handleChange}
          onGenerate={this.handleGenerateSeed}/>
        <StylishButton id='start-button' styles={Styles.startGameButton}
          text='Start Game' events={{ onClick: this.handleStart }}/>
      </div>
    );
  }
}